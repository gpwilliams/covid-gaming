...
# Q34_ = lockdown new loneliness during

# Q22_1_1, Q22_2_1 = Why play
# Q22_1, Q22_2 = percentage of games
# Q23 is gone

# Q42 is the new one about time spent in hours engaging with others with mic during
# Q43 is the new one about time spent in hours engaging with others withOUT mic during
# Q44 is the new one about time spent alone during

# removed q18 = games_single_muliplayer_before_
# removed q23 = games_single_muliplayer_during_
...
raw_data_renamed <- raw_data %>% 
  ...
  rename(
    ...
    # hours_played_during = q21_1, # removed
    hours_together_with_chat_during = q42_1,
    hours_together_without_chat_during = q43_1,
  ...
  rename_at(
    ...
    ~ str_replace(., "q38_", "das_during_")
  ) %>% 
  rename_at(
    vars(starts_with("q39_")),
    ~ str_replace(., "q39_", "loneliness_during_")
  ) %>% 
  rename_at(
    vars(starts_with("q34_")), # NEW
    ~ str_replace(., "q34_", "loneliness_extended_during_")
  ) %>%
  rename_at( # CHANGED
    ...
    ~ str_replace(., "q22_", "why_play_during_")
  ) %>% 
  rename_at(  # CHANGED
    # needs complex logic due to items being 
    # e.g. q22_1 through to q22_9 here but 
    ...
    ~ str_replace(., "q22_", "games_hours_during_")
  ) %>% 
  # clean up messy names from those with multiple numbers
  rename_at(
    .vars = vars(contains("why_play_during")),
    .funs = funs(sub("[_]1$", "", .))
  )
